(function ($) {
	...
	$(window).on('load', (function() {
		...
		// Accordion
		if($('.svm-accordion').length) {
			var _color_active = $('.svm-accordion').data('active-classes');
			var _color_inactive = $('.svm-accordion').data('inactive-classes');

			$('.svm-accordion button').on('click', (function() {
				var _group = $(this).data('group');
				var _target = $(this).data('accordion-target');

				$('.' + _group).addClass('svm-hidden');
				$(_target).removeClass('svm-hidden');
				
				$('.svm-accordion button svg').removeClass('svm-rotate-180');
				$(this).find('svg').addClass('svm-rotate-180');

				$('.svm-accordion button').removeClass(_color_active);
				$('.svm-accordion button').addClass(_color_inactive);

				$(this).addClass(_color_active);
				$(this).removeClass(_color_inactive);
			}))
		}

		// Tab
		if($('.svm-tab').length) {
			$('.svm-tab li button').on('click', (function() {
				var _tab = $(this).data('tabs-toggle');
				var _target = $(this).data('tabs-target');

				$('.svm-tab li button').removeClass('svm-active');
				$(this).addClass('svm-active');

				$(_tab + ' div[role="tabpanel"]').addClass('svm-hidden');
				$(_tab + ' ' + _target).removeClass('svm-hidden');
			}))
		}
		...
		// Playlist
		if($('.svm-playlist-item').length) {
			$('.svm-playlist-item').on('click', (function() {
				var _url = $(this).data('url');
				var _thumbnail = $(this).data('thumbnail');

				$('.svm-playlist-item').removeClass('svm-playlist-item-active');

				$(this).addClass('svm-playlist-item-active');

				$('.svm-playlist-preview .svm-open-media').attr('data-url', _url);
				$('.svm-playlist-preview img').attr('src', _thumbnail);
			}))
		}
	}))
})(jQuery);